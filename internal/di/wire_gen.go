...
func InitializeUserHandler() (*server.FiberServer, error) {
	...
	refreshtokenMiddleware := middleware.NewRefreshtokenMiddleware(jwtPkgStruct)
	middlewareDependencies := middleware.NewMiddlewareDependencies(jwtMiddleware, blackListMiddleware, permissionMiddleware, registerMiddleware, refreshtokenMiddleware)
	...
	backgroundServiceInterface := services.NewBackgroundService(queries, redisOTPRepositoryInterface)
	authServiceInterface := services.NewAuthService(otpServiceInterface, queries, jwtPkgStruct, redisOTPRepositoryInterface, backgroundServiceInterface)
	...
	errorHandler := pkg.NewErrorHandler(backgroundServiceInterface)
