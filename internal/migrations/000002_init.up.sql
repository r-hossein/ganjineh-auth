-- +migrate Up
-- init.up.sql
-- Migration generated from uploaded file: ganjineh-postgersql.sql
-- This single file contains the full "up" migration for use with golang-migrate (migrate).
-- It runs inside a transaction where supported.
...
BEGIN;
  ...
  CREATE TYPE IF NOT EXISTS order_status AS ENUM ('draft', 'pending', 'confirmed', 'processing', 'shipped', 'delivered', 'cancelled', 'refunded');
  CREATE TYPE IF NOT EXISTS order_from_type AS ENUM ('IN_PERSON', 'INSTAGRAM', 'WEBCITE', 'TELEGRAM');
  CREATE TYPE IF NOT EXISTS payment_status AS ENUM ('pending', 'paid', 'failed', 'refunded', 'partially_refunded');
  CREATE TYPE IF NOT EXISTS payment_method_type AS ENUM ('cash', 'ONLINE', 'cart');
  CREATE TYPE IF NOT EXISTS shipping_method_type AS ENUM ('IN_PERSON', 'POST', 'INTRA_CITY');
  CREATE TYPE IF NOT EXISTS company_type AS ENUM ('online_store', 'physical_store', 'multichannel_store');
  CREATE TYPE IF NOT EXISTS return_status_type AS ENUM ('requested', 'approved', 'rejected', 'completed', 'cancelled');
  CREATE TYPE IF NOT EXISTS warehouse_type AS ENUM ('ONLINE', 'SOTORE', 'WAREHOUSE');
  CREATE TYPE IF NOT EXISTS inventory_status AS ENUM ('in_stock', 'out_of_stock', 'discontinued', 'pre_order');
  CREATE TYPE IF NOT EXISTS address_type AS ENUM ('billing', 'shipping', 'both');
  CREATE TYPE IF NOT EXISTS gender_type AS ENUM ('male', 'female', 'other', 'not_specified');
  CREATE TYPE IF NOT EXISTS product_status AS ENUM ('active', 'inactive', 'draft', 'archived');
  ...
  CREATE TYPE IF NOT EXISTS inventory_change_type AS ENUM ('STOCK_IN', 'STOCK_OUT', 'ADJUSTMENT', 'CORRECTION');
...
CREATE TABLE IF NOT EXISTS user_companies (
  ...
  CREATE TRIGGER IF NOT EXISTS trg_users_updated_at BEFORE UPDATE ON users FOR EACH ROW EXECUTE FUNCTION set_updated_at();
  CREATE TRIGGER IF NOT EXISTS trg_roles_updated_at BEFORE UPDATE ON roles FOR EACH ROW EXECUTE FUNCTION set_updated_at();
  CREATE TRIGGER IF NOT EXISTS trg_companies_updated_at BEFORE UPDATE ON companies FOR EACH ROW EXECUTE FUNCTION set_updated_at();
  CREATE TRIGGER IF NOT EXISTS trg_brands_updated_at BEFORE UPDATE ON brands FOR EACH ROW EXECUTE FUNCTION set_updated_at();
  CREATE TRIGGER IF NOT EXISTS trg_categories_updated_at BEFORE UPDATE ON categories FOR EACH ROW EXECUTE FUNCTION set_updated_at();
  CREATE TRIGGER IF NOT EXISTS trg_warehouses_updated_at BEFORE UPDATE ON warehouses FOR EACH ROW EXECUTE FUNCTION set_updated_at();
  CREATE TRIGGER IF NOT EXISTS trg_products_updated_at BEFORE UPDATE ON products FOR EACH ROW EXECUTE FUNCTION set_updated_at();
  CREATE TRIGGER IF NOT EXISTS trg_customers_updated_at BEFORE UPDATE ON customers FOR EACH ROW EXECUTE FUNCTION set_updated_at();
  CREATE TRIGGER IF NOT EXISTS trg_orders_updated_at BEFORE UPDATE ON orders FOR EACH ROW EXECUTE FUNCTION set_updated_at();

  CREATE INDEX IF NOT EXISTS idx_users_phone ON users(phone_number);
  CREATE INDEX IF NOT EXISTS idx_companies_owner ON companies(owner_id);
  CREATE INDEX IF NOT EXISTS idx_products_company_slug ON products(company_id, slug);
  ...
  CREATE INDEX IF NOT EXISTS idx_inventory_available_partial ON inventory(product_id) WHERE (available_stock > 0);
